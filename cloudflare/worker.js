...
const getUsers = (env) => JSON.parse(env.MNT_USERS_JSON || "[]");
const encode = (value) => btoa(unescape(encodeURIComponent(value))).replace(/=/g, "").replace(/\+/g, "-").replace(/\//g, "_");
const decode = (value) => decodeURIComponent(escape(atob(value.replace(/-/g, "+").replace(/_/g, "/") + "===".slice((value.length + 3) % 4))));
async function sign(value, secret) { const key = await crypto.subtle.importKey("raw", new TextEncoder().encode(secret), { name: "HMAC", hash: "SHA-256" }, false, ["sign"]); const bytes = new Uint8Array(await crypto.subtle.sign("HMAC", key, new TextEncoder().encode(value))); return btoa(String.fromCharCode(...bytes)).replace(/=/g, "").replace(/\+/g, "-").replace(/\//g, "_"); }
async function tokenFor(user, env) { const body = encode(JSON.stringify({ id: user.id, exp: Date.now() + 43200000 })); return `${body}.${await sign(body, env.AUTH_SECRET)}`; }
